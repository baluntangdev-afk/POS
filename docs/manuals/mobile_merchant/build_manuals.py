# -*- coding: utf-8 -*-
"""Builds the two Cartivo Merchant manuals (User + Technical) as styled HTML,
then renders each to PDF with headless Chrome / Edge.

    cd docs/manuals/mobile_merchant
    python build_manuals.py

Output (next to this script):
    CartivoMerchant-User-Manual.html / .pdf
    CartivoMerchant-Technical-Manual.html / .pdf

Content lives in content_user.py / content_tech.py. Layout / CSS is shared in
spirit with the kiosk + mobile manual pipelines but kept self-contained here.
"""
...
VERSION = "App v1.0.0 \u00b7 Manual rev. 1"
DATE = "September 2026"
...
<div class="cover">
  <div class="kicker">Cartivo Merchant</div>
  <h1>{esc(title)}</h1>
...
def note(text, kind="note"):
    label = {"note": "Note", "warn": "Important", "ok": "Tip"}[kind]
...
if __name__ == "__main__":
    print("Building Cartivo Merchant manuals ->", OUT)
    build(
        "CartivoMerchant-User-Manual",
        "Cartivo Merchant \u2014 User Manual",
        "How to set the app up on a phone or tablet and work every screen, step by step.",
        "Merchant Staff, Store Owners & Front-of-House Operators",
        "This is a non-technical guide. It assumes the Cartivo Merchant app is "
        "already installed on the device. If it still needs installing or "
        "configuring, hand your IT person the <i>Cartivo Merchant \u2014 Technical "
        "Manual</i> first.",
        USER,
    )
    build(
        "CartivoMerchant-Technical-Manual",
        "Cartivo Merchant \u2014 Technical Manual",
        "Architecture, environment, the device-auth chain, the live-orders socket, build and deployment.",
        "Developers, Integrators & IT Staff",
        "The app lives in the <code>mobile_merchant/</code> folder of the "
        "repository. A Flutter SDK (Dart 3.11+), Android tooling, and a populated "
        "<code>.env</code> file are assumed.",
        TECH,
    )
    print("Done.")
