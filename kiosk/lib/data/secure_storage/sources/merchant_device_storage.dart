...
/// webhook-receiver: a stable per-install id (survives restarts, regenerated
/// only on reinstall / storage wipe) and the `device_id` issued by
...
class MerchantDeviceStorage {
  ...
  /// The Kiosk ID (`merchant_id`) this device was last registered against. A
  /// registration is scoped to one merchant, so a change here means the
  /// device must re-register.
  Future<String?> get registeredStoreId => _storage.read(key: _registeredStoreIdKey);
  ...
  /// POS Terminal Details dialog can show the last known state.
  Future<String?> get lastStatus => _storage.read(key: _lastStatusKey);

  Future<void> writeLastStatus(String status) => _storage.write(key: _lastStatusKey, value: status);
  ...
  Future<void> writeMerchantName(String name) => _storage.write(key: _merchantNameKey, value: name);
