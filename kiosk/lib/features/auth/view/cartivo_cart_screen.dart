...
import '../../cartivo_orders/state/cartivo_checkout_notifier.dart';
...
class CartivoCartScreen extends ConsumerWidget {
  const CartivoCartScreen({super.key});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    ref.listen(cartivoCheckoutProvider, (previous, next) {
      switch (next) {
        case AsyncData(value: final order?):
          showMessageDialog(
            ...
            title: 'Order Placed',
            message:
                'Order #${order.id} for ${formatPesoPrice(order.currency, order.total)} '
                'has been created.',
            onPrimaryPressed: () {
        ...
        case AsyncError(:final error):
          showNetworkErrorDialog(
            context,
            error: error,
            onRetry: () => ref.read(cartivoCheckoutProvider.notifier).checkout(),
          );
        default:
          break;
      }
    ...
    return Scaffold(
      ...
      appBar: CartivoAppBar(
        trailing: cartItems.isEmpty
            ? null
            : IconButton(
                onPressed: () => ref.read(cartivoCartProvider.notifier).clear(),
                icon: Icon(
        ...
        children: [
          Expanded(
            child: Padding(
              ...
              child: cartItems.isEmpty ? const _EmptyCartState() : _CartItemList(items: cartItems),
            ),
          ),
...
class _CartItemList extends StatelessWidget {
  ...
  final IList<CartivoCartItem> items;
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  '${formatPesoPrice(item.product.currency, item.product.price)} each',
                  style: TextStyle(
                ...
                Row(
                  ...
                  children: [
                    Text(
                      formatPesoPrice(item.product.currency, item.totalPrice),
                      style: TextStyle(
...
class _CartTotalBar extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final isCheckingOut = ref.watch(cartivoCheckoutProvider).isLoading;
        ...
        children: [
          Row(
            children: [
              ...
              Text(
                formatPesoPrice(currency, totalPrice),
                style: TextStyle(
          ...
          SizedBox(
            ...
            child: Button(
              ...
              onPressed: isCheckingOut
                  ? null
                  : () => ref.read(cartivoCheckoutProvider.notifier).checkout(),
            ),
          ),
        ],
      ),
