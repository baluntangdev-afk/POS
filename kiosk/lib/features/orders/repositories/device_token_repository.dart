...
abstract class DeviceTokenRepository {
  /// Returns the cached `/devices/token` bearer for [merchantId], minting a
  /// fresh one when it's missing, for another merchant, or about to expire.
  Future<String> ensureToken(String merchantId);

  /// Unconditionally mints a fresh bearer from the stored device credentials.
  /// Throws [DeviceTokenException] on failure.
  Future<String> refreshToken(String merchantId);

  /// Drops the cached bearer so the next [ensureToken] re-mints.
  Future<void> invalidate();
}

class DeviceTokenRepositoryImpl implements DeviceTokenRepository {
  const DeviceTokenRepositoryImpl(this._api, this._storage, this._deviceStorage);
  ...
  Future<String> refreshToken(String merchantId) async {
    ...
    try {
      dto = await _api.fetchToken(deviceId: deviceId, deviceSecret: deviceSecret);
    } on ApiException catch (error) {
      final reason = deviceTokenErrorFrom(error);
      throw DeviceTokenException(reason, deviceTokenMessageFrom(error, reason));
    }
