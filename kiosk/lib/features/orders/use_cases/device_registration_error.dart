...
/// Why a `POST /devices/register` attempt failed, in terms the POS Terminal
/// Details dialog can show the user.
...
/// error [reason].
String deviceRegistrationMessageFrom(Object error, DeviceRegistrationError reason) {
...
DeviceRegistrationError? _fromCode(String code) => switch (code) {
  'invalid_request' || 'invalid_device' => DeviceRegistrationError.invalidRequest,
  'unauthorized' || 'invalid_webhook_secret' || 'invalid_client' => DeviceRegistrationError.unauthorized,
  'already_registered' || 'device_exists' => DeviceRegistrationError.alreadyRegistered,
  'rate_limited' || 'too_many_requests' => DeviceRegistrationError.rateLimited,
  _ => null,
};
...
extension DeviceRegistrationErrorMessage on DeviceRegistrationError {
  String get message => switch (this) {
    DeviceRegistrationError.invalidRequest => 'Some device details are missing or invalid.',
    DeviceRegistrationError.unauthorized => "This app isn't authorized to register a device.",
    DeviceRegistrationError.alreadyRegistered => 'This device is already registered.',
    DeviceRegistrationError.rateLimited => 'Too many attempts — wait a moment and try again.',
    DeviceRegistrationError.network => 'No connection. Check your network and try again.',
    DeviceRegistrationError.unexpectedResponse => 'The server sent back something unexpected. Try again.',
    DeviceRegistrationError.serverError => 'The server ran into a problem. Try again shortly.',
    DeviceRegistrationError.unknown => "Couldn't register this device. Try again.",
  };
}
