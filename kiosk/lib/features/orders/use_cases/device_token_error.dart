...
/// in terms the UI can show the user.
...
DeviceTokenError? _fromCode(String code) => switch (code) {
  'device_pending' || 'not_approved' || 'pending_approval' => DeviceTokenError.devicePendingApproval,
  'invalid_device' ||
  'device_not_found' ||
  'unknown_device' ||
  'device_revoked' ||
  'invalid_device_secret' => DeviceTokenError.invalidDeviceCredentials,
  'invalid_request' || 'invalid_merchant' || 'unknown_merchant' => DeviceTokenError.invalidRequest,
...
/// mapped copy for [reason].
String deviceTokenMessageFrom(Object error, DeviceTokenError reason) {
  if (reason.isRecoverable && error is ApiResponseException && error.serverMessage.isNotEmpty) {
...
extension DeviceTokenErrorMessage on DeviceTokenError {
  String get message => switch (this) {
    DeviceTokenError.noDeviceCredentials =>
      "This device isn't registered yet. Live orders will connect once registration completes.",
    DeviceTokenError.devicePendingApproval =>
      "This device is still awaiting merchant approval. Live orders will connect once it's approved.",
    DeviceTokenError.invalidDeviceCredentials =>
      "This device's credentials were rejected. Re-register the device from POS Terminal Details.",
    DeviceTokenError.invalidRequest => 'The orders service rejected the connection request. Check the Kiosk ID.',
    DeviceTokenError.rateLimited => 'Too many attempts — waiting a moment before retrying.',
    DeviceTokenError.network => "Can't reach the orders service. Check your network.",
    DeviceTokenError.unexpectedResponse => 'The orders service sent back something unexpected. Retrying.',
    DeviceTokenError.serverError => 'The orders service ran into a problem. Retrying shortly.',
    DeviceTokenError.unknown => "Couldn't connect to the orders service. Retrying.",
  };
  ...
  bool get isRecoverable => switch (this) {
    DeviceTokenError.noDeviceCredentials || DeviceTokenError.invalidDeviceCredentials => false,
    _ => true,
  };
}
