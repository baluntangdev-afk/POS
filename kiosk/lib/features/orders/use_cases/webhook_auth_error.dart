...
/// Maps an [ApiException] from `OrdersAuthApi.fetchToken` to a UI-facing
/// reason. A recognized backend `error` slug wins; otherwise the HTTP status
/// decides; a transport failure is [WebhookAuthError.network].
...
WebhookAuthError? _fromCode(String code) => switch (code) {
  ...
  'invalid_client' || 'unknown_client' || 'unauthorized_client' => WebhookAuthError.invalidClient,
  'invalid_request' ||
  'invalid_merchant' ||
  'unknown_merchant' ||
...
extension WebhookAuthErrorMessage on WebhookAuthError {
  String get message => switch (this) {
    WebhookAuthError.invalidWebhookSecret =>
      'This app build has the wrong webhook secret. Reinstall with the correct configuration.',
    WebhookAuthError.invalidClient => "This app build isn't recognized by the orders service.",
    WebhookAuthError.unauthorized => "The orders service rejected this app's credentials.",
    WebhookAuthError.invalidRequest => 'The orders service rejected the sign-in request. Check the Kiosk ID.',
    WebhookAuthError.rateLimited => 'Too many attempts — wait a moment and try again.',
    WebhookAuthError.network => "Can't reach the orders service. Check your network and try again.",
    WebhookAuthError.unexpectedResponse => 'The orders service sent back something unexpected. Try again.',
    WebhookAuthError.serverError => 'The orders service ran into a problem. Try again shortly.',
    WebhookAuthError.unknown => "Couldn't connect to the orders service. Try again.",
  };
  ...
  bool get isRecoverable => switch (this) {
    WebhookAuthError.invalidWebhookSecret || WebhookAuthError.invalidClient => false,
    _ => true,
  };
}
