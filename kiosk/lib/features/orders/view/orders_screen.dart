import 'dart:async';

import 'package:fast_immutable_collections/fast_immutable_collections.dart';
import 'package:flutter/material.dart';
import 'package:flutter_hooks/flutter_hooks.dart';
import 'package:hooks_riverpod/hooks_riverpod.dart';

import '../../../styles/color_set.dart';
import '../../../styles/responsive/breakpoint.dart';
import '../../../styles/responsive/responsive_value.dart';
import '../../../theme/pos_design.dart';
import '../../../widgets/brand_header.dart';
import '../../../widgets/windows_scaffold.dart';
import '../entities/order_event.dart';
import '../state/pending_orders_count_provider.dart';
import 'order_fulfillment_spec.dart';
import 'order_status.dart';
import 'widgets/order_fulfillment_nav.dart';
import 'widgets/order_kanban_board.dart';

class OrdersScreen extends HookConsumerWidget {
  const OrdersScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final events = ref.watch(persistedOrdersProvider).value ?? const [];
    final selected = useState(OrdersFilter.all);

    return WindowsScaffold(
      backgroundColor: ColorSet.background,
      body: Column(
        children: [
          BrandHeader(
            trailing:
                events.isEmpty ? null : _DeleteAllButton(onConfirmed: () => deleteAllOrders(ref)),
          ),
          Expanded(
            child:
                events.isEmpty
                    ? const _EmptyBody()
                    : _OrdersBody(
                      events: events.toIList(),
                      selected: selected.value,
                      onSelect: (f) => selected.value = f,
                    ),
          ),
        ],
      ),
    );
  }
}

class _OrdersBody extends StatelessWidget {
  const _OrdersBody({required this.events, required this.selected, required this.onSelect});

  final IList<OrderEvent> events;
  final OrdersFilter selected;
  final ValueChanged<OrdersFilter> onSelect;

  @override
  Widget build(BuildContext context) {
    final counts = <OrdersFilter, int>{
      for (final spec in kOrderFulfillmentSpecs)
        spec.filter:
            events.where((e) => orderMatchesFilter(spec.filter, e.data.fulfillmentType)).length,
    };
    final specs =
        kOrderFulfillmentSpecs
            .where((spec) => spec.filter != OrdersFilter.other || (counts[spec.filter] ?? 0) > 0)
            .toList();

    final effectiveSelected = specs.any((s) => s.filter == selected) ? selected : OrdersFilter.all;
    final activeSpec = specs.firstWhere((s) => s.filter == effectiveSelected);

    final matching = events.where(
      (e) => orderMatchesFilter(effectiveSelected, e.data.fulfillmentType),
    );
    final active = <OrderEvent>[];
    final cancelled = <OrderEvent>[];
    var activeTotal = 0.0;
    for (final event in matching) {
      if (classifyOrderStatus(event) == OrderCardStatus.cancelled) {
        cancelled.add(event);
      } else {
        active.add(event);
        activeTotal += event.data.total;
      }
    }
    final sorted = [...active, ...cancelled].toIList();

    final r = context.responsive;
    final vertical = !context.breakpoint.isPhone;
    final nav = OrderFulfillmentNav(
      specs: specs,
      counts: counts,
      selected: effectiveSelected,
      onSelect: onSelect,
      vertical: vertical,
    );
    final board = Padding(
      padding: EdgeInsets.all(r.value<double>(kiosk: 24, tablet: 16, phone: 12)),
      child: OrderKanbanBoard(
        title: activeSpec.label,
        accentColor: activeSpec.accentColor,
        events: sorted,
        activeTotal: activeTotal,
      ),
    );

    if (!vertical) {
      return Column(
        children: [SizedBox(width: double.infinity, child: nav), Expanded(child: board)],
      );
    }

    return Row(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [nav, Expanded(child: board)],
    );
  }
}

class _DeleteAllButton extends StatelessWidget {
  const _DeleteAllButton({required this.onConfirmed});

  final VoidCallback onConfirmed;

  Future<void> _confirmDelete(BuildContext context) async {
    final confirmed = await showDialog<bool>(
      context: context,
      barrierColor: Colors.black.withValues(alpha: 0.4),
      builder:
          (dialogContext) => AlertDialog(
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(POSRadius.lg)),
            title: const Text('Delete all orders?', style: TextStyle(fontWeight: FontWeight.w700)),
            content: const Text(
              "This clears every order from this screen and can't be undone.",
              style: TextStyle(color: POSColors.textSecondary),
            ),
            actions: [
              TextButton(
                onPressed: () => Navigator.of(dialogContext).pop(false),
                child: const Text('Cancel'),
              ),
              FilledButton(
                onPressed: () => Navigator.of(dialogContext).pop(true),
                style: FilledButton.styleFrom(backgroundColor: ColorSet.danger),
                child: const Text('Delete all'),
              ),
            ],
          ),
    );
    if (confirmed ?? false) onConfirmed();
  }

  @override
  Widget build(BuildContext context) {
    return IconButton(
      onPressed: () => unawaited(_confirmDelete(context)),
      tooltip: 'Delete all orders',
      icon: const Icon(Icons.delete_outline_rounded),
      color: ColorSet.danger,
    );
  }
}

class _EmptyBody extends StatelessWidget {
  const _EmptyBody();

  @override
  Widget build(BuildContext context) {
    final r = context.responsive;
    final iconBoxSize = r.value<double>(kiosk: 96, tablet: 80, phone: 64);

    return Center(
      child: Padding(
        padding: EdgeInsets.all(r.value<double>(kiosk: 32, tablet: 24, phone: 16)),
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Container(
              width: iconBoxSize,
              height: iconBoxSize,
              decoration: BoxDecoration(
                color: ColorSet.primary.withValues(alpha: 0.1),
                borderRadius: BorderRadius.circular(POSRadius.xl),
              ),
              child: Icon(
                Icons.receipt_long_outlined,
                color: ColorSet.primary,
                size: iconBoxSize * 0.5,
              ),
            ),
            SizedBox(height: r.value<double>(kiosk: 24, tablet: 20, phone: 16)),
            Text(
              'Orders',
              style: TextStyle(
                fontSize: r.value<double>(kiosk: 24, tablet: 20, phone: 18),
                fontWeight: FontWeight.w700,
                color: POSColors.textPrimary,
                letterSpacing: -0.3,
              ),
            ),
            SizedBox(height: r.value<double>(kiosk: 8, tablet: 6, phone: 4)),
            Text(
              'Waiting for orders',
              style: TextStyle(
                fontSize: r.value<double>(kiosk: 16, tablet: 14, phone: 13),
                fontWeight: FontWeight.w500,
                color: POSColors.textSecondary,
              ),
            ),
            SizedBox(height: r.value<double>(kiosk: 12, tablet: 10, phone: 8)),
            ConstrainedBox(
              constraints: BoxConstraints(
                maxWidth: r.value<double>(kiosk: 360, tablet: 320, phone: 280),
              ),
              child: Text(
                'New online orders will show up here as they come in.',
                textAlign: TextAlign.center,
                style: TextStyle(
                  fontSize: r.value<double>(kiosk: 14, tablet: 13, phone: 12),
                  color: POSColors.textTertiary,
                ),
              ),
            ),
          ],
        ),
      ),
    );
  }
}
